...
% se=5.8e7;
inner_it = 50; outer_it = 100; tol=1e-6; % iterative solver inputs
...
%             load('prestore_3000_2000_10.mat')
            ...
            %             load('prestore_1000_1000_100.mat');% according to the structure size to choose prestored Toeplitz
...
for freq_no=1:num_freq
    ...
    for port_no=1:num_ports
        ...
        if (port_no == 1)
            ...
            lse_sparse_precon_prepare_dia(dx,freq,OneoverMc,idxS3,st_sparse_precon*ko^2,AL,dim_ind,cap_dia);
            %             lse_sparse_precon_prepare(dx,freq,OneoverMc,idxS3,st_sparse_precon,AL,dim_ind,blk_pre,ids_pre);
            disp(['Time for prepare precond ::: ' ,num2str(toc(tinisim))]);
        end
        % Define the handle for matvect
        fACPU   = @(J)lse_matvect_mult1_dia(J, fN_all,fN_ch_all, A_ind,dim_ind, OneoverMc, dx, freq,ko, idxS5,1,inds_glob,diel_pnl_id_locs,diel_pnl_ids,ids_panels,num_diels,0,fl_Tuck);
        tini = tic;
