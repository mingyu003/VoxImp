function lse_sparse_precon_prepare_TL(dx,freq,OneoverMc,idxS3,st_sparse_precon,nodeid_4_grnd,nodeid_4_injectcurr,A_ind,dim_ind,blk_pre,ids_pre,ord_pre,fl_Vsour)
...
diag_charge=st_sparse_precon(4);

inds=zeros(Nc,3);
inds(1:Nc,1)=[1:1:Nc];
inds(1:Nc,2)=inds(1:Nc,1);
inds(1:Nc,3)=abs(diag_pulse);
...
if (fl_volt_source == 1)
    A_ind_tmp=A_ind; % Attention::: temporarily doubling the memory for A_ind!!!
    A_ind_tmp(nodeid_4_grnd,:)=0;
    A_ind_tmp(nodeid_4_injectcurr,:)=0;
    Sch_comp=DD+A_ind_tmp*A_inv*A_ind';
    clear A_ind_tmp
else % symm. voltage source and current source will use this
    if fl_Vsour==1
        Sch_comp=DD+A_ind*A_inv*A_ind';
    else
        Sch_comp1=A_ind*A_inv*A_ind';
%         Sch_comp2=Aq*transpose_Aq;
        Sch_comp2=A_temp;
