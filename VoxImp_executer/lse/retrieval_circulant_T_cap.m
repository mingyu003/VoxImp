function [fN_charge]=retrieval_circulant_T_cap(dx,L,M,N,Eps_inout,tolerance,fl_Tucker_decomp,fN_charge_Toeplitz)
...
if size_stored_tensor(1) > 3 %the prestored tensor is compressed
    ...
    toni1=toc(titi1);
    [fN_ch_all{1,1,1}] = Toeplitz_JVIE_cap('G_xx_charge',L+1,M+1,N+1,dx^3*G_mn_xx_c(1:L+1,1:M,1:N)); % Gp_mn_xx
    clear G_mn_xx_c
    if (fl_no_fft == 0)
        tic
    ...
    if (fl_diel == 1)
        ...
        [fN_ch_all{1,1,2}] = Toeplitz_JVIE_cap('G_xx_Efield',L+1,M+1,N+1,dx^2*G_mn_xx_e(1:L+1,1:M,1:N)); % Gp_mn_xx
        clear G_mn_xx_e
        if (fl_no_fft == 0)
            tic
    ...
    [fN_ch_all{1,1,1}] = Toeplitz_JVIE_cap('G_xx_charge',L+1,M+1,N+1,dx^3*fN_charge_Toeplitz{1,1,1}(1:L+1,1:M,1:N)); % Gp_mn_xx
    if (fl_no_fft == 0)
        tic
        fN_ch_all{1,1,1}(:,:,:) = fftn(fN_ch_all{1,1,1}(:,:,:));
        disp(['Time for FFT of Gxx_c ::: ',num2str(toc)])
    ...
    if (fl_diel == 1)
        [fN_ch_all{1,1,2}] = Toeplitz_JVIE_cap('G_xx_Efield',L+1,M+1,N+1,dx^2*fN_charge_Toeplitz{1,1,2}(1:L+1,1:M,1:N)); % Gp_mn_xx
        if (fl_no_fft == 0)
            tic
            fN_ch_all{1,1,2}(:,:,:) = fftn(fN_ch_all{1,1,2}(:,:,:));
            disp(['Time for FFT of Gxx_d ::: ',num2str(toc)])
...
if size_stored_tensor(1) > 3 %the prestored tensor is compressed
    ...
    toni2=toc(titi2);
    [fN_ch_all{2,2,1}] = Toeplitz_JVIE_cap('G_yy_charge',L+1,M+1,N+1,dx^3*G_mn_yy_c(1:L,1:M+1,1:N)); % Gp_mn_yy
    clear G_mn_yy_c
    if (fl_no_fft == 0)
        tic
    ...
    if (fl_diel == 1)
        ...
        [fN_ch_all{2,2,2}] = Toeplitz_JVIE_cap('G_yy_Efield',L+1,M+1,N+1,dx^2*G_mn_yy_e(1:L,1:M+1,1:N)); % Gp_mn_yy
        clear G_mn_yy_e
        if (fl_no_fft == 0)
            tic
    ...
    [fN_ch_all{2,2,1}] = Toeplitz_JVIE_cap('G_yy_charge',L+1,M+1,N+1,dx^3.*fN_charge_Toeplitz{2,2,1}(1:L,1:M+1,1:N)); % Gp_mn_yy
    if (fl_no_fft == 0)
        tic
        fN_ch_all{2,2,1}(:,:,:) = fftn(fN_ch_all{2,2,1}(:,:,:));
        disp(['Time for FFT of Gyy_c ::: ',num2str(toc)])
    ...
    if (fl_diel == 1)
        [fN_ch_all{2,2,2}] = Toeplitz_JVIE_cap('G_yy_Efield',L+1,M+1,N+1,dx^2*fN_charge_Toeplitz{2,2,2}(1:L,1:M+1,1:N)); % Gp_mn_yy
        if (fl_no_fft == 0)
            tic
            fN_ch_all{2,2,2}(:,:,:) = fftn(fN_ch_all{2,2,2}(:,:,:));
            disp(['Time for FFT of Gyy_d ::: ',num2str(toc)])
...
if size_stored_tensor(1) > 3 %the prestored tensor is compressed
    ...
    toni3=toc(titi3);
    [fN_ch_all{3,3,1}] = Toeplitz_JVIE_cap('G_zz_charge',L+1,M+1,N+1,dx^3*G_mn_zz_c(1:L,1:M,1:N+1)); % Gp_mn_zz
    clear G_mn_zz_c
    if (fl_no_fft == 0)
        tic
    ...
    if (fl_diel == 1)
        ...
        [fN_ch_all{3,3,2}] = Toeplitz_JVIE_cap('G_zz_Efield',L+1,M+1,N+1,dx^2*G_mn_zz_e(1:L,1:M,1:N+1)); % Gp_mn_zz
        clear G_mn_zz_e
        if (fl_no_fft == 0)
            tic
    ...
    [fN_ch_all{3,3,1}] = Toeplitz_JVIE_cap('G_zz_charge',L+1,M+1,N+1,dx^3.*fN_charge_Toeplitz{3,3,1}(1:L,1:M,1:N+1)); % Gp_mn_zz
    if (fl_no_fft == 0)
        tic
        fN_ch_all{3,3,1}(:,:,:) = fftn(fN_ch_all{3,3,1}(:,:,:));
        disp(['Time for FFT of Gzz_c ::: ',num2str(toc)])
    ...
    if (fl_diel == 1)
        [fN_ch_all{3,3,2}] = Toeplitz_JVIE_cap('G_zz_Efield',L+1,M+1,N+1,dx^2*fN_charge_Toeplitz{3,3,2}(1:L,1:M,1:N+1)); % Gp_mn_zz
        if (fl_no_fft == 0)
            tic
            fN_ch_all{3,3,2}(:,:,:) = fftn(fN_ch_all{3,3,2}(:,:,:));
            disp(['Time for FFT of Gzz_d ::: ',num2str(toc)])
...
if size_stored_tensor(1) > 3 %the prestored tensor is compressed
    ...
    toni4=toc(titi4);
    [fN_ch_all{1,2,1}] = Toeplitz_JVIE_cap('G_xy_charge',L+1,M+1,N+1,dx^3*G_mn_xy_c(1:L+2,1:M+2,1:N)); % Gp_mn_xy
    clear G_mn_xy_c
    if (fl_no_fft == 0)
        tic
    ...
    if (fl_diel == 1)
        ...
        [fN_ch_all{1,2,2}] = Toeplitz_JVIE_cap('G_xy_Efield',L+1,M+1,N+1,dx^2*G_mn_xy_e(1:L+2,1:M+2,1:N)); % Gp_mn_xy
        clear G_mn_xy_e
        if (fl_no_fft == 0)
            tic
    ...
    [fN_ch_all{1,2,1}] = Toeplitz_JVIE_cap('G_xy_charge',L+1,M+1,N+1,dx^3.*fN_charge_Toeplitz{1,2,1}(1:L+2,1:M+2,1:N)); % Gp_mn_xy
    if (fl_no_fft == 0)
        tic
        fN_ch_all{1,2,1}(:,:,:) = fftn(fN_ch_all{1,2,1}(:,:,:));
        disp(['Time for FFT of Gxy_c ::: ',num2str(toc)])
    ...
    if (fl_diel == 1)
        [fN_ch_all{1,2,2}] = Toeplitz_JVIE_cap('G_xy_Efield',L+1,M+1,N+1,dx^2*fN_charge_Toeplitz{1,2,2}(1:L+2,1:M+2,1:N)); % Gp_mn_xy
        if (fl_no_fft == 0)
            tic
            fN_ch_all{1,2,2}(:,:,:) = fftn(fN_ch_all{1,2,2}(:,:,:));
            disp(['Time for FFT of Gxy_d ::: ',num2str(toc)])
...
if size_stored_tensor(1) > 3 %the prestored tensor is compressed
    ...
    toni5=toc(titi5);
    [fN_ch_all{1,3,1}] = Toeplitz_JVIE_cap('G_xz_charge',L+1,M+1,N+1,dx^3*G_mn_xz_c(1:L+2,1:M,1:N+2)); % Gp_mn_xz
    clear G_mn_xz_c
    if (fl_no_fft == 0)
        tic
    ...
    if (fl_diel == 1)
        ...
        [fN_ch_all{1,3,2}] = Toeplitz_JVIE_cap('G_xz_Efield',L+1,M+1,N+1,dx^2*G_mn_xz_e(1:L+2,1:M,1:N+2)); % Gp_mn_xz
        clear G_mn_xz_e
        if (fl_no_fft == 0)
            tic
    ...
    [fN_ch_all{1,3,1}] = Toeplitz_JVIE_cap('G_xz_charge',L+1,M+1,N+1,dx^3.*fN_charge_Toeplitz{1,3,1}(1:L+2,1:M,1:N+2)); % Gp_mn_xz
    if (fl_no_fft == 0)
        tic
        fN_ch_all{1,3,1}(:,:,:) = fftn(fN_ch_all{1,3,1}(:,:,:));
        disp(['Time for FFT of Gxz_c ::: ',num2str(toc)])
    ...
    if (fl_diel == 1)
         [fN_ch_all{1,3,2}] = Toeplitz_JVIE_cap('G_xz_Efield',L+1,M+1,N+1,dx^2*fN_charge_Toeplitz{1,3,2}(1:L+2,1:M,1:N+2)); % Gp_mn_xz
         if (fl_no_fft == 0)
            tic
            fN_ch_all{1,3,2}(:,:,:) = fftn(fN_ch_all{1,3,2}(:,:,:));
            disp(['Time for FFT of Gxz_d ::: ',num2str(toc)])
...
if size_stored_tensor(1) > 3 %the prestored tensor is compressed
    ...
    toni6=toc(titi6);
    toni=toni1+toni2+toni3+toni4+toni5+toni6
    [fN_ch_all{2,3,1}] = Toeplitz_JVIE_cap('G_yz_charge',L+1,M+1,N+1,dx^3*G_mn_yz_c(1:L,1:M+2,1:N+2)); % Gp_mn_yz
    clear G_mn_yz_c
    if (fl_no_fft == 0)
        tic
    ...
    if (fl_diel == 1)
        ...
        [fN_ch_all{2,3,2}] = Toeplitz_JVIE_cap('G_yz_Efield',L+1,M+1,N+1,dx^2*G_mn_yz_e(1:L,1:M+2,1:N+2)); % Gp_mn_yz
        clear G_mn_yz_e
        if (fl_no_fft == 0)
            tic
    ...
    [fN_ch_all{2,3,1}] = Toeplitz_JVIE_cap('G_yz_charge',L+1,M+1,N+1,dx^3.*fN_charge_Toeplitz{2,3,1}(1:L,1:M+2,1:N+2)); % Gp_mn_yz
    if (fl_no_fft == 0)
        tic
        fN_ch_all{2,3,1}(:,:,:) = fftn(fN_ch_all{2,3,1}(:,:,:));
        disp(['Time for FFT of Gyz_c ::: ',num2str(toc)])
    ...
    if (fl_diel == 1)
         [fN_ch_all{2,3,2}] = Toeplitz_JVIE_cap('G_yz_Efield',L+1,M+1,N+1,dx^2*fN_charge_Toeplitz{2,3,2}(1:L,1:M+2,1:N+2)); % Gp_mn_yz
         if (fl_no_fft == 0)
            tic
            fN_ch_all{2,3,2}(:,:,:) = fftn(fN_ch_all{2,3,2}(:,:,:));
            disp(['Time for FFT of Gyz_d ::: ',num2str(toc)])
...
if (fl_diel == 1)
    ...
    if size_stored_tensor(1) > 3 %the prestored tensor is compressed
        ...
        [fN_ch_all{2,1,2}] = Toeplitz_JVIE_cap('G_yx_Efield',L+1,M+1,N+1,dx^2*G_mn_yx_e(1:L+2,1:M+2,1:N)); % Gp_mn_yx
        clear G_mn_yx_e
    else                         %the prestored tensor is not compressed
        [fN_ch_all{2,1,2}] = Toeplitz_JVIE_cap('G_yx_Efield',L+1,M+1,N+1, dx^2*fN_charge_Toeplitz{2,1,2}(1:L+2,1:M+2,1:N)); % Gp_mn_yx
    end
    ...
    if size_stored_tensor(1) > 3 %the prestored tensor is compressed
        ...
        [fN_ch_all{3,1,2}] = Toeplitz_JVIE_cap('G_zx_Efield',L+1,M+1,N+1,dx^2*G_mn_zx_e(1:L+2,1:M,1:N+2)); % Gp_mn_zx
        clear G_mn_zx_e
    else                         %the prestored tensor is not compressed
        [fN_ch_all{3,1,2}] = Toeplitz_JVIE_cap('G_zx_Efield',L+1,M+1,N+1,dx^2*fN_charge_Toeplitz{3,1,2}(1:L+2,1:M,1:N+2)); % Gp_mn_zx
    end
    ...
    if size_stored_tensor(1) > 3 %the prestored tensor is compressed
        ...
        [fN_ch_all{3,2,2}] = Toeplitz_JVIE_cap('G_zy_Efield',L+1,M+1,N+1,dx^2*G_mn_zy_e(1:L,1:M+2,1:N+2)); % Gp_mn_zx
        clear G_mn_zy_e
    else                         %the prestored tensor is not compressed
        [fN_ch_all{3,2,2}] = Toeplitz_JVIE_cap('G_zy_Efield',L+1,M+1,N+1,dx^2*fN_charge_Toeplitz{3,2,2}(1:L,1:M+2,1:N+2)); % Gp_mn_zy
    end
