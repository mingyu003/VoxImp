function lse_sparse_precon_prepare(dx,freq,OneoverMc,idxS3,st_sparse_precon,Ae,dim_ind,blk_pre,ids_pre)
global A_inv LL UU PP QQ RR slct_decomp_sch fl_cholmod Sch_sparse

%slct_decomp_sch='ldlt_decomp';
%%'chol_decomp','no_decomp','ldlt_decomp','chol_decomp','AGMG'
...
% tic
% for ll=1:size(ids_pre,1)
%     A_temp(ids_pre{ll},ids_pre{ll})=blk_pre{ll};
% end
% toc
% tic
row_ind=cell(size(ids_pre,1),1);
col_ind=cell(size(ids_pre,1),1);
A_temp_val=cell(size(ids_pre,1),1);
for ll=1:size(ids_pre,1)
    row_ind{ll}= repmat(ids_pre{ll},length(ids_pre{ll}),1);
    col_ind{ll}= repelem(ids_pre{ll},length(ids_pre{ll}));
    A_temp_val{ll}=reshape(blk_pre{ll},[length(ids_pre{ll})*length(ids_pre{ll}) 1]);
end

row_ind=cell2mat(row_ind);
col_ind=cell2mat(col_ind);
A_temp_val=cell2mat(A_temp_val);
A_temp=sparse(row_ind,col_ind,A_temp_val,Nn,Nn);
% toc

% norm(A_temp(:)-A_temp1(:))/norm(A_temp(:))
...
%         spparms('spumoni',2)
        ...
        memestimated = (infomem1.bytes)/(1024*1024);
        disp(['Memory for Schur complement matrix (MB)::' , num2str(memestimated)]);
        b_temp=rand(Nn,2);%+sqrt(-1)*rand(Nn,1);
