...
function [JOut_full_out]=lse_sparse_precon_multiply(JOut_full_in,Ae,Aq,blk_pre,ids_pre,ord_pre)
...
%             1,1e-4,100,[],[],202);
        ...
        a_temp=strumpack_solve([real(bb_dum) imag(bb_dum)]);
%         b_temp=strumpack_solve(imag(bb_dum));
        JOut_full_out(num_curr+num_node+1:num_curr+2*num_node)=a_temp(:,1)+sqrt(-1).*a_temp(:,2);
